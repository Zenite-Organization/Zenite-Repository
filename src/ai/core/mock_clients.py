...
class MockVectorStoreClient:
    ...
    def __init__(self):
        ...
        self.issues = [
            {"id": "1", "number": 101, "title": "Integrar API de pagamentos", "body": "Criar integração com provedor X usando OAuth2 e webhooks.", "description": "Criar integração com provedor X", "estimated_hours": 10, "real_hours": 14, "state": "closed", "labels": ["payments", "backend"], "created_at": "2025-08-01T10:12:00Z", "updated_at": "2025-08-10T12:00:00Z", "url": "https://github.com/org/repo/issues/101", "user": {"login": "alice"}, "assignees": [{"login": "bob"}], "comments": 3, "repository": "org/repo"},
            {"id": "2", "number": 102, "title": "Corrigir bug de autenticação", "body": "Token de sessão expira indevidamente causando logout do usuário.", "description": "Token expira indevidamente", "estimated_hours": 4, "real_hours": 3.5, "state": "closed", "labels": ["bug", "auth"], "created_at": "2025-07-15T09:00:00Z", "updated_at": "2025-07-16T11:00:00Z", "url": "https://github.com/org/repo/issues/102", "user": {"login": "carol"}, "assignees": [], "comments": 2, "repository": "org/repo"},
            {"id": "3", "number": 103, "title": "Refatorar módulo de relatórios", "body": "Separar camadas, otimizar consultas e reduzir tempo de geração de relatórios.", "description": "Separar camadas e otimizar consultas", "estimated_hours": 20, "real_hours": 26, "state": "closed", "labels": ["refactor", "performance"], "created_at": "2025-06-10T14:20:00Z", "updated_at": "2025-06-30T16:00:00Z", "url": "https://github.com/org/repo/issues/103", "user": {"login": "dan"}, "assignees": [{"login": "erin"}], "comments": 5, "repository": "org/repo"},
            {"id": "4", "number": 104, "title": "Adicionar testes unitários", "body": "Cobertura para serviços críticos do domínio de cobrança.", "description": "Cobertura para serviços críticos", "estimated_hours": 8, "real_hours": 7, "state": "closed", "labels": ["tests", "quality"], "created_at": "2025-05-01T08:00:00Z", "updated_at": "2025-05-20T10:00:00Z", "url": "https://github.com/org/repo/issues/104", "user": {"login": "frank"}, "assignees": [], "comments": 1, "repository": "org/repo"},
            {"id": "5", "number": 105, "title": "Ajuste UI responsiva", "body": "Corrigir layout em mobile e ajustar breakpoints.", "description": "Corrigir layout em mobile", "estimated_hours": 6, "real_hours": 5.5, "state": "closed", "labels": ["frontend", "ui"], "created_at": "2025-04-12T12:30:00Z", "updated_at": "2025-04-15T09:00:00Z", "url": "https://github.com/org/repo/issues/105", "user": {"login": "gina"}, "assignees": [{"login": "hank"}], "comments": 4, "repository": "org/repo"},
            {"id": "6", "number": 106, "title": "Adicionar suporte a múltiplas moedas", "body": "Permitir transações em USD, EUR e BRL com conversão e rounding.", "description": "Suporte a múltiplas moedas", "estimated_hours": 12, "real_hours": 15, "state": "closed", "labels": ["payments", "feature"], "created_at": "2025-03-01T10:00:00Z", "updated_at": "2025-03-25T13:00:00Z", "url": "https://github.com/org/repo/issues/106", "user": {"login": "ian"}, "assignees": [{"login": "jane"}], "comments": 6, "repository": "org/repo"},
            {"id": "7", "number": 107, "title": "Melhorar logs e tracing", "body": "Adicionar structured logging e tracing distribuído para diagnósticos.", "description": "Logs e tracing", "estimated_hours": 5, "real_hours": 6, "state": "closed", "labels": ["observability"], "created_at": "2025-02-18T11:40:00Z", "updated_at": "2025-02-28T12:00:00Z", "url": "https://github.com/org/repo/issues/107", "user": {"login": "kate"}, "assignees": [], "comments": 0, "repository": "org/repo"},
            {"id": "8", "number": 108, "title": "Criar endpoint de relatórios CSV", "body": "Exportar relatórios em CSV paginados para análises externas.", "description": "Endpoint CSV para relatórios", "estimated_hours": 7, "real_hours": 9, "state": "closed", "labels": ["feature", "reports"], "created_at": "2025-01-10T09:10:00Z", "updated_at": "2025-01-20T10:00:00Z", "url": "https://github.com/org/repo/issues/108", "user": {"login": "liam"}, "assignees": [{"login": "mia"}], "comments": 2, "repository": "org/repo"},
            {"id": "9", "number": 109, "title": "Atualizar dependências de segurança", "body": "Atualizar bibliotecas críticas para corrigir vulnerabilidades.", "description": "Atualizar dependencies", "estimated_hours": 3, "real_hours": 4, "state": "closed", "labels": ["maintenance", "security"], "created_at": "2024-12-01T07:00:00Z", "updated_at": "2024-12-05T08:00:00Z", "url": "https://github.com/org/repo/issues/109", "user": {"login": "noah"}, "assignees": [], "comments": 1, "repository": "org/repo"},
            {"id": "10", "number": 110, "title": "Documentar fluxo de deploy", "body": "Criar documentação e playbook para deploys em produção.", "description": "Documentação de deploy", "estimated_hours": 2, "real_hours": 2.5, "state": "closed", "labels": ["docs", "devops"], "created_at": "2024-11-01T08:30:00Z", "updated_at": "2024-11-03T09:00:00Z", "url": "https://github.com/org/repo/issues/110", "user": {"login": "olivia"}, "assignees": [], "comments": 0, "repository": "org/repo"},
        ]
...
class MockLLMClient:
    ...
    def send_prompt(self, prompt: str, **kwargs) -> str:
        ...
        # Analogical: tenta extrair real_hours da lista de issues no prompt
        if "real:" in p or "real_hours" in p or "issues similares" in p:
            nums = [float(x) for x in re.findall(r"real[:=]?\s*(\d+(?:\.\d+)?)", prompt)]
            if not nums:
                nums = [it.get("real_hours") for it in [ ] if it]
            if nums:
            ...
            # fallback
            import json
            return json.dumps({"estimate_hours": 8.0, "confidence": 0.4, "justification": "Fallback analógico: sem similares extraídos."})
